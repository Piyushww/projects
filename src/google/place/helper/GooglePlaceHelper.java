...
public class GooglePlaceHelper {
	...
	public ArrayList<HashMap<String, Object>> getPlaces(String url,
			int activityFlag) {
		...
		if (json != null) {
			...
			try {
				...
				// JSONObject html1 = json.getJSONObject("html_attributions");
				resultJson = json.getJSONArray("results");
				...
				for (int i = 0; i < resultJson.length(); i++) {
					...
					JSONObject locationJson = geometry
							.getJSONObject("location");

					String lat = locationJson.getString(TAG_LAT);
					Log.d("", "lat: " + lat);
					location.put("lat" + i, Double.valueOf(lat));
					String lng = locationJson.getString(TAG_LONG);
					Log.d("", "lng: " + lng);
					location.put("lng" + i, Double.valueOf(lng));
					...
					singleResult.put("location" + i, location);
					singleResult.put("id" + i, id);
					singleResult.put("name" + i, name);
					singleResult.put("icon" + i, icon);
					singleResult.put("reference" + i, reference);
					singleResult.put("type" + i, types);
					singleResult.put("vicinity" + i, vicinity);

					if (activityFlag == 2) {

						try {
							String formatted_address = c
									.getString(TAG_FORMATTED_ADD);
							singleResult.put("formatted_address",
									formatted_address);
						...
						try {
							...
							JSONObject opening_time_obj = opening_time_obj_main
									.getJSONObject("open");
							String opening_time = opening_time_obj
									.getString("time");
							String closing_time = closing_time_obj
									.getString("time");
								...
								for (int aspects_length = 0; aspects_length < aspects
									...
									ratings.put(
											...
											""
													+ single_aspects_obj
															.getInt("rating"));
									ratings.put("type", single_aspects_obj
											.getString("type"));
